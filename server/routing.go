...
func RegisterController(name string, e *echo.Echo, m []echo.HandlerFunc, dal DataAccessLayer, config Config) {
...
func RegisterRoutes(e *echo.Echo, config map[string][]echo.HandlerFunc, dal DataAccessLayer, serverConfig Config) {

	// Batch Support
	batch := NewBatchController(dal)
	batchHandlers := make([]echo.HandlerFunc, len(config["Batch"]))
